...
export async function POST(request: Request) {
  try {
    ...
    const recommendedModules = Array.isArray(generated.recommended_modules)
      ? generated.recommended_modules.filter((id): id is string => typeof id === 'string' && allowedIds.has(id))
      : []
    ...
    const { error: interviewError } = await serviceSupabase.from('interviews').upsert({
      id: interviewId, user_id: user.id, title, status: 'completed', progress: 100,
      messages: cleanAnswers, created_at: now, updated_at: now,
    })
    ...
    const { data: project, error: projectError } = await serviceSupabase.from('projects').insert({
      id: projectId, user_id: user.id, interview_id: interviewId, title, content,
    ...
    return NextResponse.json({
      success: true,
      project,
      capabilities: recommendedModules.map((id) => HLAI_CAPABILITIES.find((capability) => capability.id === id)).filter(Boolean),
    })
  } catch (error) {
